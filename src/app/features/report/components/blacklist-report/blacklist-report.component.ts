...
@Component({
  selector: 'app-blacklist-report',
  templateUrl: './blacklist-report.component.html',
  styleUrls: ['./blacklist-report.component.css']
})
export class BlacklistReportComponent {
  ...
  getReport() {
    ...
    this.reportService
      .getBlackListRpt(startDate, endDate, parkingId)
      .toPromise()
      .then((data) => {
  ...
  onExporting(e: any) {
    ...
    const worksheet = workbook.addWorksheet('Lista negra por fecha')
    ...
    const titleRow = worksheet.addRow(['', '', '', 'Reporte - Lista negra de parqueo'])
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      saveAs(
        blob,
        `Reporte de blacklist - Generado - '${this.now.toLocaleString()}.xlsx`
      )
    })
    e.cancel = true
  }
