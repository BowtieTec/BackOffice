...
isApproved(payment: any) {
  ...
  getBiltmoreRpt(initDate: string, endDate: string, parqueo: string) {
    const _initDate = new Date(initDate).toISOString().split('T')[0]
    const _endDate = new Date(endDate).toISOString().split('T')[0]
    return this.http
      .get<ResponseModel>(
        `${this.apiUrl}backoffice/report/getBiltmoreRpt/dates?initDate=${_initDate}&endDate=${_endDate}&parqueo=${parqueo}`
      )
      .pipe(
        map((res) => {
          console.log(res);
